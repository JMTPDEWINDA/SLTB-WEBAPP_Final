...
const generateReferenceNo = () => {
  ...
  return `SLTB-REPLANT-${timestamp}${random}`;
};

// @route   POST /api/replanting/apply
// @desc    Submit replanting subsidy application
// @access  Private
router.post('/apply', authenticateToken, [
  ...
  body('replanting_type').notEmpty().withMessage('Replanting type is required'),
  ...
  try {
    ...
    const [existingFiles] = await pool.execute(
      'SELECT id FROM replanting_applications WHERE file_no = ?',
      [file_no]
    );
    ...
    await pool.execute(
      ...
      [reference_no, 'replanting', result.insertId]
    );

    res.status(201).json({
      success: true,
      message: 'Replanting application submitted successfully',
      data: {
...
// @route   GET /api/replanting/applications
// @desc    Get user's replanting applications
// @access  Private
...
// @route   GET /api/replanting/applications/:id
// @desc    Get specific replanting application
// @access  Private
...
// @route   PUT /api/replanting/applications/:id
// @desc    Update replanting application
// @access  Private
router.put('/applications/:id', authenticateToken, [
  ...
  try {
    ...
    const [applications] = await pool.execute(
      'SELECT id, status FROM replanting_applications WHERE id = ? AND user_id = ?',
      [id, req.user.id]
    );
    ...
    if (updateData.amount_per_plant || updateData.approved_plants) {
      const [currentApp] = await pool.execute(
        'SELECT amount_per_plant, approved_plants FROM replanting_applications WHERE id = ?',
        [id]
      );
    ...
    const [result] = await pool.execute(
      `UPDATE replanting_applications SET ${updateFields.join(', ')} WHERE id = ?`,
      updateValues
    );
...
// @route   DELETE /api/replanting/applications/:id
// @desc    Delete replanting application
// @access  Private
router.delete('/applications/:id', authenticateToken, async (req, res) => {
  try {
    ...
    // Delete application
    const [result] = await pool.execute(
      'DELETE FROM replanting_applications WHERE id = ? AND user_id = ?',
      [id, req.user.id]
    );
